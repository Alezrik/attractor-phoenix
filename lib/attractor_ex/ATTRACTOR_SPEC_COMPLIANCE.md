...
7. CSS/JSON stylesheet parsing and parser handling of quoted-string separators, quoted comment markers, CSS comments, and invalid selector diagnostics: `model_stylesheet_test.exs` and `parser_test.exs`.
...
4. Full linting parity matrix (coverage now includes start/exit structure, reachability, edge-target existence, retry-target reference checks, explicit type/fidelity linting, `wait.human` timeout/default, timeout-format, and duplicate-key checks, stylesheet syntax/diagnostics, plus custom lint rule API via `Validator.validate/2`).
5. HTTP server mode.
