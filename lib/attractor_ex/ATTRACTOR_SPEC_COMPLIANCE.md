...
| `6. Human-in-the-Loop` | `partial` | `wait.human` supports context-driven answers, timeout/default handling, blank-answer default routing, unmatched-answer fallback to the first outgoing choice, skip handling, and interviewer abstractions (`AutoApprove`, `Console`, `Callback`, `Queue`, `Recording`) via handler options. The interviewer behaviour now includes optional `ask_multiple/4` and `inform/4` callbacks, and the console interviewer surfaces node prompt, default choice, and timeout metadata during interactive selection. Full upstream interviewer UX parity remains open. |
