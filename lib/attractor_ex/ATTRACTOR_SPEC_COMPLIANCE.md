...
| `7. Validation and Linting` | `partial` | Core diagnostics exist (start/exit, edges, condition parse, unreachable/dead-end flow warnings, retry target reference checks, goal gate retry hints, codergen prompt warning) plus `wait.human` choice/default checks, codergen LLM attr lint checks (`reasoning_effort`, `temperature`, `max_tokens`, provider/model pairing), and `validate/2` custom lint rule hooks. Full lint parity matrix remains open. |
| `8. Model Stylesheet` | `partial` | CSS-like `model_stylesheet` parsing is implemented (`*`, `.class`, `#id` with declaration blocks), including specificity ordering and same-specificity last-rule wins behavior, and is applied before validation. Backward-compatible JSON stylesheet parsing remains supported, along with extended selector support (`type=...`, `node[type=...]`, compound selectors, selector lists). Lint coverage for stylesheet-applied codergen LLM attrs is now present via validator checks, while stricter syntax/property parity remains open. |
...
2. Strict stylesheet parity details (for example tighter CSS grammar/property validation and richer stylesheet-focused lint diagnostics).
