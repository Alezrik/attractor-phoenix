...
4. Full linting parity matrix (coverage now includes start/exit structure, reachability, edge source/target existence, retry-target reference checks, explicit type/fidelity linting, `wait.human` prompt/default/timeout checks, ambiguous default-choice lint, duplicate-key checks, stylesheet syntax/diagnostics, plus custom lint rule API via `Validator.validate/2`).
5. HTTP server mode.
