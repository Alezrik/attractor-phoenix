...
| `Appendix A/B/C/D` | `partial` | Major attrs, shape mapping, status contract, and error categories are represented, including graph-level `tool_hooks.pre` / `tool_hooks.post`, `stack.child_workdir`, Appendix C `status.json` contract fields (`outcome`, `preferred_next_label`, `suggested_next_ids`, `context_updates`, `notes`) with backward-compatible aliases, and Appendix D runtime error categorization surfaced on outcomes, terminal results, and failure events. Not every appendix item is fully implemented. |
