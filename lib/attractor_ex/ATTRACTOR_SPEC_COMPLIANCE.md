...
3. Full interviewer UX/interaction parity beyond the current named interfaces (`AutoApprove`, `Console`, `Callback`, `Queue`, `Recording`, `Server`) and optional `ask_multiple`/`inform` hooks, especially richer structured answer payloads across every interface beyond the current question metadata normalization (`required`, `input_mode`) and timeout parsing coverage.
