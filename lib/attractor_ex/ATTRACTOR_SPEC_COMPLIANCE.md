...
|---|---|---|
| `2. DOT DSL Schema` | `partial` | Supports directed graphs, attrs, chained edges, order-sensitive node/edge defaults, multi-line attr blocks, class attr, shape mapping, quoted node identifiers, single/double-quoted attr values, value parsing, quote-aware statement splitting (so `;` and newlines inside quoted values are preserved), quote-aware comment stripping (so `//` and `/* */` inside quoted attr values are preserved), and recursive inline/nested `subgraph` flattening with scoped default inheritance plus subgraph-label-derived classes. Full grammar coverage is not fully implemented. |
...
9. HTTP server mode, SSE events, pending question flow, answer submission, graph format variants, and Phoenix-hosted integration helpers: `http_test.exs` and `phoenix_http_integration_test.exs`.
...
5. Full HTTP server mode parity details such as richer graph rendering beyond SVG/DOT/JSON and production-grade service concerns beyond the implemented core endpoint set.
...
mix test test/attractor_ex/http_test.exs test/attractor_ex/phoenix_http_integration_test.exs
mix precommit
```
