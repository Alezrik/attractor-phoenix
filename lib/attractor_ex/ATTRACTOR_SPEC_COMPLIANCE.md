...
| `6. Human-in-the-Loop` | `partial` | `wait.human` supports context-driven answers, timeout/default handling, blank-answer default routing, invalid-answer retry behavior, and interviewer abstractions (`AutoApprove`, `Console`, `Callback`, `Queue`, `Recording`) via handler options. The interviewer behaviour now includes optional `ask_multiple/4` and `inform/4` callbacks, and the console interviewer surfaces node prompt, default choice, and timeout metadata during interactive selection. Full upstream interviewer UX parity remains open. |
...
8. Human interviewer adapters, recording wrapper, optional multi-select/inform callbacks, and console prompt metadata: `handlers_test.exs` and `interviewers_test.exs`.
...
3. Full interviewer UX/interaction parity beyond the current named interfaces (`AutoApprove`, `Console`, `Callback`, `Queue`, `Recording`) and optional `ask_multiple`/`inform` hooks.
